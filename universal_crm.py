...
with tabs[0]:
    ...
    if not acts: st.info("Configurez une activité.")
        ...
        if cols:
            sel_col = st.selectbox("Modèle", [c['name'] for c in cols])
            mod = next(c for c in cols if c['name'] == sel_col)
            fields = mod['fields']
...
# ONGLET 2 : GESTION RESTAURÉE (FORMS + FICHIERS)
with tabs[1]:
    st.header("Gestion des Dossiers")
    my_acts = supabase.table("activities").select("id").eq("company_id", MY_COMPANY_ID).execute().data
    if my_acts:
        all_cols = supabase.table("collections").select("*").in_("activity_id", [a['id'] for a in my_acts]).execute().data
        if all_cols:
            recs = supabase.table("records").select("*, collections(name, fields)").in_("collection_id", [c['id'] for c in all_cols]).order('created_at', desc=True).execute().data
            if recs:
                ...
                if sel:
                    ...
                    # 3. PDF
                    if all_urls and st.button("📄 FUSIONNER EN PDF"):
...
# ONGLET 3 : CONFIGURATION (DRAG & DROP RESTAURÉ)
if "3. ⚙️ Configuration" in tabs_list:
    ...
    with tabs[idx]:
        ...
        acts = supabase.table("activities").select("*").eq("company_id", MY_COMPANY_ID).execute().data
        if acts:
            s_act = st.selectbox("Gérer les modèles de :", [a['name'] for a in acts])
            ...
            with st.expander("➕ Nouveau Modèle"):
                ...
                fn, ft = c1.text_input("Nom champ"), c2.selectbox("Type", ["Texte Court", "SIRET", "Fichier/Image", "Section/Titre"])
                ...
                with st.expander(f"📝 {m['name']} (Gérer/Trier)"):
                    ...
                    if st.button("💾 Valider l'ordre", key=f"save_{m['id']}"):
                        ...
                        for l in sorted_labels:
                            new_f.append(next(f for f in m['fields'] if f"{f['name']} [{f['type']}]" == l))
                        supabase.table("collections").update({"fields": new_f}).eq("id", m['id']).execute()
                        st.success("Ordre mis à jour !")
                    
                    if st.button("🗑️ Supprimer le modèle", key=f"kil_{m['id']}"):
                        supabase.table("collections").delete().eq("id", m['id']).execute()
                        st.rerun()

# ONGLET 4 : UTILISATEURS (ACTIONS FONCTIONNELLES)
            ...
            for user in u_list:
                if user['id'] != st.session_state.user.id:
                    ...
                    if (MY_ROLE == "admin1") or (MY_ROLE == "admin2" and user['role'] == "user"):
                        if st.button(f"🗑️ Supprimer {user['email']}", key=f"d_{user['id']}"):
                            try:
                                supabase.table("profiles").delete().eq("id", user['id']).execute()
                                st.success("Supprimé !")
                                time.sleep(0.5)
                                st.rerun() # INDISPENSABLE POUR QUE LE TABLEAU S'ACTUALISE
